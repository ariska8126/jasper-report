...
public interface AttendanceRepository extends JpaRepository<Attendance, String> {
    ...
    @Query(value = "SELECT s.attendance_date AS date, time_format(s.attendance_time, \"%H:%i\") as start, "
            ...
            + "WHERE attendance_type = 'start' AND a.user_id= ?1 AND a.attendance_date "
            + "BETWEEN ?2 AND ?3) s JOIN (SELECT attendance_date, attendance_time, "
            + "attendance_remark FROM attendance WHERE attendance_type = 'end' "
            + "AND user_id= ?1 AND attendance_date BETWEEN ?2 AND ?3) e ON s.attendance_date = e.attendance_date",
            nativeQuery = true)
    public List<AttendanceProjections> findAllAttendance(@Param("userId") String userId,
